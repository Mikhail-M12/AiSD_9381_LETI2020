...
Node* Node::insertRoot(Node* p, int k) // вставка нового узла с ключом k в корень дерева p
{
    ...
    {
        cout << "\n" << k << " < " << "значения узла " << p->key << ", поэтому переходим к левому поддереву\n";
    ...
    {
        cout << "\n" << k << " > " << "значения узла " << p->key << ", поэтому переходим к правому поддереву\n";
...
Node* Node::insert(Node* p, int k) // рандомизированная вставка нового узла с ключом k в дерево p
{
    ...
    if(k == p->key){//если узел с таким значением уже есть в дереве, увеличиваем значение поля amount
        cout << "\nЕще одна попытка вставить узел со значением " << k << endl;
        p->amount++;
        return p;
    }
    ...
    if(rand() % (p->size+1) == 0) {  // вставка в корень происходит с вероятностью 1/(n+1),
        ...
        cout << "\n \033[31m Вставляем узел со значением " << k << " в корень дерева! \033[0m\n";
...
void Node::print2DUtil(Node *root, int space)  //печать лежачего 2D дерева
{
    if (!root)
        return;

    space += COUNT; //счетчик дистанции между уровнями
    print2DUtil(root->right, space); //обработка правого поддерева
    cout << endl;
    for (int i = COUNT; i < space; i++)
        cout << "  ";
    cout<< root->key <<"\n"; //печать значения узла
    print2DUtil(root->left, space); //обработка левого поддерева
}
