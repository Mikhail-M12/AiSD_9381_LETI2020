...
public:
    ...
    void reHash() {
        std::cout << "Таблица заполнена более чем на 80%, удваиваем размер и пересобираем" << '\n';
...
int main() {
    ...
    while (command != 6) {
        ...
        std::cout << "4. Открыть файл" << (file.is_open() ? " (сейчас открыт)" : " (сейчас закрыт)") << '\n';
        ...
        switch (command) {
            ...
            case 2:
                ...
                for (const auto &word : elements) {
                    ...
                    std::cout << "Количество элемента " << word << " в таблице: " << count << "\n\n";
                }
